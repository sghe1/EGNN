...
# Add egnn-pytorch subdirectory to path for imports
script_dir = os.path.dirname(os.path.abspath(__file__))
egnn_pytorch_dir = os.path.join(script_dir, 'egnn-pytorch')
if egnn_pytorch_dir not in sys.path:
...
class MeshEGNN(nn.Module):
    ...
    def __init__(self, in_dim=12, hidden_dim=128, depth=4, num_nodes_avg=1271):
        """
        Args:
            in_dim: Input feature dimension (12: [mesh_pos(3), world_pos(3), node_type(2), stress(1), velocity(3)])
    ...
    def forward(self, feats, coors, adj_mat, v_init=None):
        ...
        Args:
            feats: (B, N, 12) - NORMALIZED features [mesh_pos(3), world_pos(3), node_type(2), stress(1), velocity(3)]
            coors: (B, N, 3) - NORMALIZED node coordinates (centered world positions)
            adj_mat: (B, N, N) or (N, N) or (T, N, N) - adjacency matrix (sparse connectivity, may be per-timestep)
        ...
        # Handle per-timestep adjacency matrices: use first timestep if (T, N, N)
        if len(adj_mat.shape) == 3 and adj_mat.shape[0] > 1:
            adj_mat = adj_mat[0]  # Use first timestep adjacency (or could use current timestep)
        
        # Verify input feature dimension
        assert feats.shape[-1] == 12, f"Expected 12D features, got {feats.shape[-1]}D"
        ...
        node_type_one_hot = feats[:, :, 6:8]  # (B, N, 2) - extract node_type from features (indices 6-7 in 12D features)
