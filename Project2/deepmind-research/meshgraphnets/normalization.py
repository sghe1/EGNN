...
class Normalizer(snt.Module):
  ...
  def _build(self, batched_data, is_training=True):
  ...
  def __call__(self, batched_data, is_training=True):
    """Make Normalizer callable - routes to _build."""
    return self._build(batched_data, is_training=is_training)
