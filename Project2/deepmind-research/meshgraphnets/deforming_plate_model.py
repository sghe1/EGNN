...
class Model(snt.Module):
  ...
  def __init__(self, learned_model, name='Model'):
    ...
    # node features: velocity (3) + node_type (9) + stress (1) = 13
    self._node_normalizer = normalization.Normalizer(
        size=3+common.NodeType.SIZE+1, name='node_normalizer')
    # edge features: relative_world_pos (3) + length (1) + relative_mesh_pos (3) + length (1) = 8
  ...
  def _build_graph(self, inputs, is_training):
    ...
    # Include stress as a node feature
    stress = inputs['stress']
    node_features = tf.concat([velocity, node_type, stress], axis=-1)

    # construct graph edges from quads
    senders, receivers = common.quads_to_edges(inputs['cells'])
  ...
  def __call__(self, inputs):
    """Make Model callable - routes to _build."""
