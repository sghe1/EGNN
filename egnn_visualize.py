...
def main():
    ...
    if train_mode == 'overfit':
        ...
        dataset = EGNNTFRecordDataset(
            ...
            preprocessed_dir=cfg['data']['preprocessed_dir'],
            transform=transform,
            max_trajs=traj_idx + 5 # Ensure we load enough to get to traj_idx
        )
    ...
    if cells is None: raise ValueError("Cells not found.")

    # 5. Rollout
    # We ignore the 'start_step' logic inside rollout for now (assumes starting at t=0)
    # If you want to start rollout at t=50, you need to update the rollout loop offset.
